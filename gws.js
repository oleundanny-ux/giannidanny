...
import { giveawayStartEmbed, giveawayEndEmbed, giveawayRerollEmbed, errorEmbed, successEmbed, } from "../utils/embeds.js";
...
export async function execute(interaction) {
    ...
    if (sub === "create") {
        ...
        await interaction.reply({
            embeds: [
                successEmbed("Giveaway pokrenut!", `🎉 Giveaway za **${prize}** je počeo u ovom kanalu!`),
            ],
            ephemeral: true,
        });
        ...
        logger.info({ giveawayId: msg.id, prize, durationMin, winnersCount }, "Giveaway created");
        return;
    }
    if (sub === "end") {
        ...
        if (!giveaway) {
            await interaction.reply({
                embeds: [
                    errorEmbed("Giveaway nije pronađen", "Provjeri ID poruke — giveaway možda već nije aktivan ili je ID pogrešan."),
                ],
                ephemeral: true,
            });
            return;
    ...
    if (sub === "reroll") {
        ...
        if (!giveaway) {
            await interaction.reply({
                embeds: [
                    errorEmbed("Giveaway nije pronađen", "Provjeri ID poruke."),
                ],
                ephemeral: true,
            });
            return;
        ...
        await interaction.editReply({
            embeds: [
                successEmbed("Reroll završen", `Novi pobednici za **${giveaway.prize}** su odabrani!`),
            ],
        });
