...
function shipPercent(id1: string, id2: string): number {
  const seed = [...`${id1}${id2}`].reduce((a, c) => a + c.charCodeAt(0), 0);
  return ((seed * 7 + 13) % 101 + 100) % 101;
}

function lovePercent(id1: string, id2: string): number {
  const seed = [...`${id1}love${id2}`].reduce((a, c) => a + c.charCodeAt(0), 0);
  return ((seed * 11 + 37) % 101 + 100) % 101;
}

function heartBar(percent: number): string {
  const filled = Math.round(percent / 10);
  return "❤️".repeat(filled) + "🖤".repeat(10 - filled);
}

function shipLabel(percent: number): string {
  if (percent >= 90) return "💍 Savršen par!";
  if (percent >= 75) return "💕 Jako dobro!";
  if (percent >= 60) return "😊 Solidno!";
  if (percent >= 40) return "🤔 Možda...";
  if (percent >= 25) return "😬 Teško...";
  return "💔 Nema šanse!";
}

function loveLabel(percent: number): string {
  if (percent >= 90) return "💘 Totalno zaljubljen/a!";
  if (percent >= 75) return "💓 Jako voli!";
  if (percent >= 60) return "💛 Sviđa mu/joj se!";
  if (percent >= 40) return "💙 Simpatija!";
  if (percent >= 20) return "🌱 Možda nešto...";
...
const SOCIAL_ACTIONS: Record<string, ActionDef> = {
  hug: {
    color: BotColors.HUG,
    emoji: "🤗",
    withTarget: [
...
function formatText(template: string, from: User, to?: User): string {
  return template
...
export function registerLoveCommands(client: Client): void {
  client.on("messageCreate", async (message: Message) => {
    ...
    try {
      if (command === "ship") {
        ...
        const embed = new EmbedBuilder()
          .setColor(BotColors.SHIP as ColorResolvable)
      ...
      if (command === "love") {
        ...
        const embed = new EmbedBuilder()
          .setColor(BotColors.LOVE as ColorResolvable)
      ...
      if (command === "marry") {
        ...
        const embed = new EmbedBuilder()
          .setColor(BotColors.MARRY as ColorResolvable)
          .setTitle("💍 Vjenčanje!")
          .setDescription(
            `🎊 **${message.author.displayName}** i **${mentioned.displayName}** su sada **u braku**!\n\n` +
      ...
      if (command === "divorce") {
        ...
        const embed = new EmbedBuilder()
          .setColor(0x6c757d as ColorResolvable)
          .setTitle("💔 Razvod")
          .setDescription(
      ...
      if (command === "partner") {
          ...
          await message.channel.send({
            embeds: [
              new EmbedBuilder()
                .setColor(0x6c757d as ColorResolvable)
                .setDescription(
                  `💔 **${targetUser.displayName}** trenutno nije u braku.`,
                ),
            ],
          });
          return;
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `**${targetUser.displayName}** je u braku sa **${partner.displayName}**! 💒`,
            )
      ...
      if (command === "fuck") {
        ...
        const embed = new EmbedBuilder()
          .setColor(BotColors.ROAST as ColorResolvable)
          .setTitle("🚀 Odleti!")
          .setDescription(
