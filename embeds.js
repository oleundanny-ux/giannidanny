...
export function infoEmbed(title, description) {
    return new EmbedBuilder()
        .setColor(BotColors.BLURPLE)
        .setTitle(`ℹ️ ${title}`)
        .setDescription(description)
        .setTimestamp()
        .setFooter({ text: BOT_FOOTER });
}
export function raidAlertEmbed(guild, joinCount) {
    return new EmbedBuilder()
        .setColor(BotColors.RED)
        ...
        .addFields({ name: "🏰 Server", value: guild.name, inline: true }, { name: "👥 Mass Join", value: `${joinCount} korisnika`, inline: true }, {
...
export function raidUnlockEmbed(guild) {
    return new EmbedBuilder()
        .setColor(BotColors.GREEN)
        .setTitle("✅ Lockdown završen — Server je ponovo otvorен")
        .setDescription(`> Server **${guild.name}** je automatski otključan nakon 5 minuta.\n> Raid zaštita ostaje aktivna.`)
        .setTimestamp()
        .setFooter({ text: "🛡️ Anti-Raid System • Guardian Bot" });
}
export function inviteDeleteEmbed(member, channelName) {
    const tag = member instanceof GuildMember ? member.user.tag : member.tag;
    const avatar = member instanceof GuildMember
        ? member.user.displayAvatarURL()
        : member.displayAvatarURL();
    return new EmbedBuilder()
        .setColor(BotColors.ORANGE)
        .setTitle("🔗 Invite Link Blokiran")
        .setDescription(`> Korisnik **${tag}** je pokušao da pošalje invite link u kanalu **#${channelName}**.\n> Poruka je automatski obrisana.`)
        .setThumbnail(avatar)
        .setTimestamp()
        .setFooter({ text: "🛡️ Anti-Invite System • Guardian Bot" });
}
export function giveawayStartEmbed(prize, winners, endTime, hostedBy) {
...
export function giveawayEndEmbed(prize, winners) {
    const winnersText = winners.length > 0
        ? winners.map((w) => `<@${w}>`).join(", ")
        : "Nema pobednika (niko nije reagovao)";
    return new EmbedBuilder()
        .setColor(BotColors.GOLD)
...
export function giveawayRerollEmbed(prize, newWinners) {
    const winnersText = newWinners.length > 0
        ? newWinners.map((w) => `<@${w}>`).join(", ")
        : "Nema pobednika";
    return new EmbedBuilder()
        .setColor(BotColors.CYAN)
...
export function eventsEmbed(guild, events) {
    ...
    if (events.length === 0) {
        embed.setDescription("> Trenutno nema zakazanih događaja na ovom serveru.");
        return embed;
    }
    embed.setDescription(`> Pronađeno **${events.length}** predstojeći(h) događaja na serveru **${guild.name}**\n\u200b`);
